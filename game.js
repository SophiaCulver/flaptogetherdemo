...
class Bird {
    ...
    draw() {
        ...
        ctx.rotate(this.rotation * DEGREE);  //rotates to the updated rotation angle
        if(this.sleeping>1000){ctx.globalAlpha = 0.7
            ctx.drawImage(this.sprite, -this.width / 2, -this.height / 2, this.width, this.height);
            ctx.globalAlpha = 0.5
            ctx.drawImage(pause, -20, -20, 50, 50)
            if (user === true) {
                ctx.fillText(this.username.toLowerCase(), 0 + (this.height * 0.1), -this.radius - 5);
            }
        }
        else{ctx.drawImage(this.sprite, -this.width / 2, -this.height / 2, this.width, this.height);
    ...
    update(delta) {  //all properties are appropriately updated based on their conditions before redrawing can occur    
        ...
        if (this.y + this.height / 2 >= cvs.height - fg.height) {
            ...
            if (this.speed >= jump) {
                ...
                //has fully finished its flap arc and is now in a nosedive
                this.frame = 1;
            }


            else {
                this.rotation = -25;       //is currently mid flapping arc and is thus tilted upward.
...
var fg = {
    ...
    draw: function () {
        ...
        if (state.current === 2) { ctx.drawImage(pause, cvs.width / 2 - 180, cvs.height / 2 - 180, 360, 360) }


    },
    update: function (delta) {
        if (state.current === state.game) {this.x = this.x<=-cvs.width ? 0 : this.x-pace*delta }
    },
}
//--------------------------------------------------------------------------
let powerups = []
...
class Powerup {
    ...
    update(delta) {
        ...
        if (this.x + this.width <= 0) { powerups.shift() }
    }
...
class Pipes {
    ...
    draw() {
        ...
        if(hb===true)
        {
            ctx.beginPath();
            ctx.strokeStyle = "#FF0000"; //makes the stroke red
            ctx.rect(this.x+this.width*0.184, this.y, this.width*0.6312, this.height*0.944);
            ctx.rect(this.x, this.y+this.height*0.944, this.width, this.height*0.056);
            ctx.rect(this.x+this.width*0.184, this.y + this.height*1.056 + this.gap, this.width*0.6312, this.height*0.944);
            ctx.rect(this.x, this.y + this.height + this.gap, this.width, this.height*0.056);
            ctx.stroke();
        }
    }
    ...
    update(delta) {  //all properties are appropriately updated based on their conditions before redrawing can occur       
        ...
        if (this.x + this.width <= 0) { position.shift(); collided = false; passed=false; if(pace<0.5){pace += 0.0005}}

        //COLLISION DETECTION
        if (state.current === 1) {
            if (myBird.x + myBird.height * 0.1 + myBird.radius > this.x
                && myBird.x + myBird.height * 0.1 - myBird.radius < this.x + this.width
                && myBird.y + myBird.radius > this.y + this.height * 0.944
            ...
            if (myBird.x + myBird.height * 0.1 + myBird.radius > this.x
                ...
                && myBird.y - myBird.radius < this.y + this.height + this.gap + this.height*0.056) {
                if (!collided) {
                    console.log("Collision B");
                    collided = true;
                    if (!testing) {
            ...
            if (myBird.x + myBird.height * 0.1 + myBird.radius > this.x + this.width*0.184
                && myBird.x + myBird.height * 0.1 - myBird.radius < this.x + this.width*0.816
        ...
        //PASS DETECTION
        if (this.x + this.width <= myBird.x - myBird.radius / 2) {
            console.log
        if (!passed) {
         myBird.score++; }
        passed = true;}    
